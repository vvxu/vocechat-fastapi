...
from app.crud.sys_vocechat import *
...
class MessageHandler:
    ...
    def handle(self):
        ...
        # 如果有数据库，预设一个参数，避免报错。
        try:
            voce_insert_chat_mode(self.user_id)
        except:
            pass

        # 重复消息晒出
        if self.get_user_current_chat_mode() == "1":
            if voce_find_something("voce_temp_msg", f"created_at='{self.created_at}'"):
                return
            else:
                voce_insert_msg("voce_temp_msg", self.created_at, self.user_id, "user", self.msg)
    ...
    def switch_chat_mode(self, chat_mod):
        res = voce_update_chat_mode(self.user_id, chat_mod)
        if res["code"] == 1:
            return self.send_to_voce_bot(f"{res}")
        chat_mod_id = self.get_user_current_chat_mode()
    ...
    def clear_chat_history(self):
        res = voce_delete_something("voce_temp_msg", self.user_id)
        if res["code"] == 0:
            return self.send_to_voce_bot("聊天记录清理成功")
        return self.send_to_voce_bot("聊天记录清理失败")
    ...
    def clear_user_ai_system(self):
        voce_update_user_system(self.user_id, None)
    ...
    def set_user_ai_system(self):
        ...
        voce_update_user_system(self.user_id, ai_system[1].replace("\n", ""))
    ...
    def select_server_store_ai_system_to_user(self):
        ai_system = self.msg.split("+")
        if not voce_find_something("voce_server_store_system", f"system_name='{ai_system[1]}'"):
            return self.send_to_voce_bot(f"没有预制该角色")
        ai_system_find = voce_find_something("voce_server_store_system", f"system_name='{ai_system[1]}'")[0]
        voce_update_user_system(self.user_id, ai_system_find["system_descript"])
        return self.send_to_voce_bot(f"设置{ai_system_find['system_name']}成功")
    ...
    def process_user_message(self):
        ...
        if response != "超时错误，请重试！":
            if self.get_user_current_chat_mode() == "1":
                voce_insert_msg("voce_temp_msg", self.created_at, self.user_id, "assistant", response)
    ...
    def get_user_current_chat_mode(self):
        try: 
            user_info = voce_find_something("voce_chat_mode", f"user_id='{self.user_id}'")[0]
    ...
    def get_user_current_ai_system(self):
        user_info = voce_find_something("voce_chat_mode", f"user_id='{self.user_id}'")[0]
        user_current_ai_system = user_info["system"]
        if not user_current_ai_system:
            return
        return user_current_ai_system

    # 获取当期服务器中的角色情况
    def get_server_current_ai_system(self):
        try:
            results = voce_find_all("voce_server_store_system")
            return results
        except:
            return None
    ...
    def process_user_history_msg(self):
        find_msg = voce_find_something("voce_temp_msg", f"user_id='{self.user_id}'")
        history_msg_list = []
        for i in find_msg:
